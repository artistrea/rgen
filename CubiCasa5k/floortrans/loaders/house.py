"""
This file was updated for our purposes
"""

import math
import numpy as np
from CubiCasa5k.floortrans.loaders.svg_utils import PolygonWall, get_polygon, calc_distance, get_room_number, get_icon, get_icon_number, get_points, get_direction, get_gaussian2D
...
class House:
    def __init__(self, path, height, width, icon_list=icons_selected, room_list=rooms_selected):
        ...
        self.walls.fill(0)
        # self.doors = np.empty((height, width), dtype=np.uint8)
        # self.doors.fill(0)
        self.wall_ids = np.empty((height, width), dtype=np.uint8)
        self.wall_ids.fill(0)
        self.door_ids = np.empty((height, width), dtype=np.uint8)
        self.door_ids.fill(0)
        self.window_ids = np.empty((height, width), dtype=np.uint8)
        self.window_ids.fill(0)
        self.room_ids = np.empty((height, width), dtype=np.uint8)
        self.room_ids.fill(0)
        ...
        for e in svg.getElementsByTagName('g'):
            ...
            if e.getAttribute("id") == "Window":
                ...
                # UPDATE:
                window = PolygonWall(e, window_id, shape)
                window.rr, window.cc = self._clip_outside(window.rr, window.cc)
                self.window_objs.append(window)
                # self.windows[window.rr, window.cc] = room_list["Wall"]
                self.window_ids[window.rr, window.cc] = window_id
                self.window_ends.append(window.end_points)
            ...
            if e.getAttribute("id") == "Door":
                ...
                # UPDATE:
                door = PolygonWall(e, door_id, shape)
                door.rr, door.cc = self._clip_outside(door.rr, door.cc)
                self.door_objs.append(door)
                # self.doors[door.rr, door.cc] = room_list["Wall"]
                self.door_ids[door.rr, door.cc] = door_id
                self.door_ends.append(door.end_points)
                ...
                if direction == 'H':
                    ...
                    self.representation['doors'].append(door_rep)
                    # UPDATE:
                    self.representation['actual_damned_doors'].append(door_rep)
                else:
                    up_index = np.argmin(locs[:, 1])
                    up1 = locs[up_index]
                    locs = np.delete(locs, up_index, axis=0)
            ...
            if "Space " in e.getAttribute("class"):
                ...
                if len(rr) != 0:
                    ...
                    if len(rr) != 0 and len(cc) != 0:
                        # UPDATE:
                        room = PolygonWall(e, room_id, shape, only_xy=True)
                        room.rr, room.cc = self._clip_outside(room.rr, room.cc)
                        self.room_objs.append(room)
                        self.room_ids[room.rr, room.cc] = room_id
